...
path_z = zeros(size(path_x2));
position = [path_x2, path_y2, path_z];
...
for i=1:length(path_x2)
    ...
    dist(i) = sqrt(((path_x2(i+1)- path_x2(i))^2)+(path_y2(i+1)- path_y2(i))^2); 
end
...
% for i=1:length(path_x2)
%     if path_y2(i) == -25 && path_x2(i) >= -30.5 && path_x2(i) <= -14.5
%         yaw_temp(i) = 0;
%     else 
%         if path_x2(i) == -14.5 && path_y2(i) >= -20.5 && path_y2(i) <= 25.5
%             yaw_temp(i) = 0;
%         else
%             if path_x2(i) >= -10 && path_x2(i) <= 5.5 && path_y2(i) == 25.5
%                 yaw_temp(i) = 0;
%             else
%                 if path_x2(i) == 5.5 && path_y2(i) <=20 && path_y2(i) >= -25.5
%                     yaw_temp(i) = 0;
%                 else
%                     if path_x2(i) >= 10 && path_x2(i) <= 25.5 && path_y2(i) == -25.5
%                        yaw_temp(i) = 0; 
%                     else
%                         if path_x2(i) == 25.5 && path_y2(i) >= -20 && path_y2(i) <= 25
%                             yaw_temp(i) = 0; 
%                         end
%                     end
%                 end
...
for i=1:length(path_x2)
    if path_y2(i) >=11 && path_y2(i) <=21 && path_x2(i) >= -25 && path_x2(i) <=-18
        yaw_temp(i) = 60;
    else
        if path_y2(i) <= 21 && path_y2(i) >=-25 && path_x2(i) > -18 && path_x2(i) < -4.5
            yaw_temp(i) = 0;
        else
            if path_y2(i) > -25 && path_y2(i) <=-6 && path_x2(i) >= -4.5 && path_x2(i) <=10
                yaw_temp(i) = 60;
            end
        end
    end
...
%     deviation_controller_method(count) = sqrt(((robotCurrentPose(1)- dynamic_x(closestIndex_x(count)))^2)+(robotCurrentPose(2)- dynamic_y(closestIndex_y(count)))^2);
    ...
    [minValue_x,closestIndex_x(count)] = min(abs(robotCurrentPose(1)-path_x1));
    [minValue_y,closestIndex_y(count)] = min(abs(robotCurrentPose(2)-path_y1));
...
%     deviation_fusion_method(count) = sqrt(((estPosition(1)- dynamic_x(imu_closestIndex_x(count)))^2)+(estPosition(2)- dynamic_y(imu_closestIndex_y(count)))^2);
    ...
    [imu_minValue_x,imu_closestIndex_x(count)] = min(abs(estPosition(1)-path_x2));
    [imu_minValue_y,imu_closestIndex_y(count)] = min(abs(estPosition(2)-path_y2));
    deviation_fusion_method(count) = sqrt(((estPosition(1)- path_x2(imu_closestIndex_x(count)))^2)+(estPosition(2)- path_y2(imu_closestIndex_y(count)))^2);
    
    pure_pursuit_pos = [pure_pursuit_pos; robotCurrentPose(1) robotCurrentPose(2)];
...
%     plot(path1(:,1), path1(:,2),"g--")
    ...
    plot(pure_pursuit_pos(:,1), pure_pursuit_pos(:,2), '-b', 'LineWidth', 2);
