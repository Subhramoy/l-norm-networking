...
%     theta = zeros(size(path_x2));
    ...
    if mod(i,2) == 0
        ...
        speed(i) = 1.3 -(i/10);   % Giving unique speeds to each robot 
...
for i=1:robot_PP_numbers
    ...
    controller{i}.DesiredLinearVelocity = 0.6-(i/10); % Each robot gets a unique linear velocity
...
while( distanceToGoal >= goalRadius )
    ...
    for i = 1:robot_PP_numbers
        ...
        % Update the current pose
            [robotCurrentPose{i},distanceToGoal,vel,closestIndex_xRobot(i),closestIndex_yRobot(i),STASnr_rndd,closestIndex_xRobot_mrc,closestIndex_yRobot_mrc] = PurePursuitPos_path_mrc(i,...
                Params,robot(i),robotCurrentPose{i},count,snr_per_mcs_5GHz_indoor_channelB,controller{i},AP_x,AP_y,STASnr_rndd,closestIndex_xRobot(i),closestIndex_yRobot(i), ...
                closestIndex_xRobot_mrc,closestIndex_yRobot_mrc);
